...
function imagesChargees(images) {
  ...
  */
  ...
  let renderVS=`
    ...
    gl_PointSize = dataA.z*64.0;
// dataA.z = ? = texture2D(physicsData,SLOT0).z
    //dataA.z = nombre aléatoire entre 0 et 1
// dataB.z = ? = texture2D(physicsData,SLOT1).z
    //dataBN.z = nombre entier de 0 à dedede-1
       gl_PointSize=64.0;
    }
  `;
...
slot  0     1     0     1
      ...
      */
      ...
      if (slot == POSITION_SLOT) {
        ...
        float x = (time-timeDebut);
        float y = 1.0 - x * alea0to1;

        float transparence = float( dataBz < a) + float( dataBz>=a) * ( -dataBz+b)/(b-a);
    //transparence = 
        //luminosite = transparence ;
       luminosite = luminosite * 0.95;
    luminosite = luminosite * alea0to1;
        //luminosite = y;
    ...
    void main() {
      ...
      if (whichLine==ygrec || whichLine==(ygrec+1.0)) {
        ...
        if (slot==1) {
          ...
          hasard_x_0_1 = rand( gl_FragCoord.xy + sin(time+53.654543) * 150.0);
          hasard_y_0_1 = rand( gl_FragCoord.xy + sin(time) * 100.0);
          ...
          float alea2 = rand( gl_FragCoord.xy + sin(time) * 100.0);
          alea1=time;
          ...
          z = rand( gl_FragCoord.xy + sin(time) * 100.0);
     float y_hasard_marche_bien = rand( 
       gl_FragCoord.xy + 
         vec2(  
           sin(time*3.65)*77.0,
           sin(time*12.67+254.6) * 80.0
         )
      );
