...
var fbT1, fbT3, fbT5, fbT6;

var positionGl, positionTex;
  ...
  //  CSW=10;
   ...
   //  var tempoCanvas = document.getElementById("tempocanvas");
    ...
    for (k = 0; k < NBRE; k++) {
        ...
        x = (2 * vertices[k * 2] - CSW + 1) / CSW;
        y = (2 * vertices[k * 2 + 1] - CSH + 1) / -CSH;
        ...
        colorUint[k * 4 + 0] = colors[k * 4 + 0];
        colorUint[k * 4 + 1] = colors[k * 4 + 1];
        colorUint[k * 4 + 2] = colors[k * 4 + 2];
        colorUint[k * 4 + 3] = colors[k * 4 + 3];
    ...
    for (k = 0; k < NBRE; k++) {
        ...
        //tabTemp.push( (i) / PVS  , (j) / PVS ) ;
        // textureGeometry.push( (2*i +1) / (2*PVS)  , (2*j+1) / (2*PVS) ) ;
    ...
    main1Shader.destination =       gl.getUniformLocation(main1Shader, "destination");
...
function animate() {
    ...
    gl.bindFramebuffer( gl.FRAMEBUFFER , fbT6);
    gl.viewport(0, 0, PVS, PVS);// the viewport is not change
    gl.uniform1i(copieShader.toCopy,TEXTURE_UNIT_pv);
    gl.drawArrays(gl.POINTS , 0 , NBRE);
